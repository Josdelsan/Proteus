...
>
    ...
    <xsl:template match="object[contains(@classes,'change-request')]">
        ...
        <div id="{@id}" data-proteus-id="{@id}">
            ...
            <table class="change_request remus_table">
                ...
                <xsl:call-template name="generate_header">
                    ...
                    <xsl:with-param name="class"   select="'change-request'"/>
                </xsl:call-template>
                ...
                <xsl:variable name="mandatory_properties">,description,analysis,trace-type,</xsl:variable>
    ...
    <!-- ============================================= -->
    <!-- generate_object_information template          -->
    <!-- ============================================= -->

    <!-- TODO: This could be placed in utilities file  -->

    <xsl:template name="generate_object_information">
        <xsl:param name="object_id"/>

        <xsl:variable name="object" select="//object[@id=$object_id]" />
        <xsl:variable name="object_code" select="$object/properties/*[@name=':Proteus-code']" />
        <xsl:variable name="object_name">
            <xsl:if test="$object_code">
                [<xsl:value-of select="$object_code"/>]
            </xsl:if>
            <xsl:value-of select="$object/properties/*[@name=':Proteus-name']" />
        </xsl:variable>

        <xsl:if test="$object">
                <a href="#{$object_id}" onclick="selectAndNavigate(`{$object_id}`, event)">
