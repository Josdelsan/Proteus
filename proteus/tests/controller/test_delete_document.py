...
def test_delete_document_command_redo(
    ...
    for source_id in sources_ids:
        ...
        source: Object = sample_project_service._get_element_by_id(
            source_id
        )  # Source is always an object
...
def test_delete_document_command_undo(
    ...
    for source_id in sources_ids:
        source_dict: Dict[ProteusID, TraceProperty] = {}
        source = sample_project_service._get_element_by_id(source_id) # Source is always an object
        for trace in source.get_traces():
            # Check if the trace is tracing the object or its children
            common_ids = [id for id in trace.value if id in object_and_children_ids]
            if len(common_ids) > 0:
    ...
    for source_id, traces in sources_traces_before.items():
        for trace_name, trace in traces.items():
            ...
            source_updated_trace = source.get_property(trace_name)
            assert (
                source_updated_trace.value == trace.value
